...
from cv_key_lookup import lookup # Lookup table for labels
...
def _convert_to_example(filename, image_buffer, label, text, height, width):
	...
	Args:
		...
		label: dict of class names to identifiers for the ground truth
		text: dict of class names to unique human-readable labels, e.g. 'dog'
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
	...
	Args:
		...
		texts: dict of lists of strings; each string is the class, e.g. 'dog'
		labels: dict of lists of integer; each integer identifies the index.
		num_shards: integer number of shards for this data set.
	Note that each dict is organized as {'classname' : list of texts/labels}
	"""
	...
	for s in range(num_shards_per_batch):
		...
		if not os.path.exists(FLAGS.output_directory):
			try:
				...
				print('WARNING: Tried to mkdir existing dir')
...
def _process_image_files(name, filenames, texts, labels, num_shards):
	...
	Note that each dict is organized as {'classname' : list of texts/labels}
	"""
	assert all([len(filenames) == len(v) for k,v in texts.iteritems()])
	assert all([len(filenames) == len(v) for k,v in labels.iteritems()])
...
def _find_image_files(data_dir):
	...
	Args:
		data_dir: string, path to the root directory of data JSONs.
	...
	for k, v in texts.iteritems():
		texts[k] = [v[i] for i in shuffled_index]
	for k, v in labels.iteritems():
		labels[k] = [v[i] for i in shuffled_index]

	print('Found %d JSON files inside %s.' % (len(filenames), data_dir))
